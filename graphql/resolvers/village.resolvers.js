...
const resolvers = {
  Query: {
    ...
    getVillagesByTalukaId: async (_, { taluka_id, pagination = {} }) => {
      const { page = 1, limit = 1000, sortBy = "name", sortOrder = "ASC" } = pagination;
      const offset = (page - 1) * limit;
      const query = `SELECT * FROM villages WHERE taluka_id = $1 AND is_deleted = FALSE ORDER BY ${sortBy} ${sortOrder} LIMIT $2 OFFSET $3;`;
      const result = await pool.query(query, [taluka_id, limit, offset]);
      return result.rows;
    ...
    getVillageStatsByTaluka: async (_, { taluka_id, district_id, activePagination = {}, deletedPagination = {} }) => {
        ...
        const [talukasResult, villagesResult, deletedVillagesResult, countResult] = await Promise.all([
          pool.query(`SELECT id, name, gu_name FROM talukas WHERE district_id = $1 AND is_deleted = FALSE ORDER BY name ASC`, [selectedDistrictId]),
      
          pool.query(`
            SELECT id, name, gu_name FROM villages
            WHERE taluka_id = $1 AND is_deleted = FALSE
            ORDER BY ${activePagination.sortBy || 'name'} ${activePagination.sortOrder || 'ASC'}
            LIMIT $2 OFFSET $3
          `, [
            selectedTalukaId,
          ...
          pool.query(`
            ...
            ORDER BY ${deletedPagination.sortBy || 'name'} ${deletedPagination.sortOrder || 'ASC'}
            LIMIT $2 OFFSET $3
          `, [
            selectedTalukaId,
  ...
  Mutation: {
    ...
    softDeleteVillage: async (_, { id }) => {
      const result = await pool.query(`UPDATE villages SET is_deleted = TRUE WHERE id = $1 RETURNING *;`, [id]);
      return result.rows[0];
    },

    softDeleteVillages: async (_, { ids }) => {
      const results = await Promise.all(
        ids.map(id => pool.query(`UPDATE villages SET is_deleted = TRUE WHERE id = $1 RETURNING *;`, [id]))
      );
      return results.map(r => r.rows[0]);
    },

    restoreVillage: async (_, { id }) => {
      const result = await pool.query(`UPDATE villages SET is_deleted = FALSE WHERE id = $1 RETURNING *;`, [id]);
      return result.rows[0];
    },

    restoreVillages: async (_, { ids }) => {
      const results = await Promise.all(
        ids.map(id => pool.query(`UPDATE villages SET is_deleted = FALSE WHERE id = $1 RETURNING *;`, [id]))
      );
      return results.map(r => r.rows[0]);
    },

    hardDeleteVillage: async (_, { id }) => {
      const result = await pool.query(`DELETE FROM villages WHERE id = $1;`, [id]);
